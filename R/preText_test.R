...
#' using a rank-based comparison of document distance matrices.
#'
#' @param distance_matrices A list of document distance matrices generated by the
#' \code{scaling_comparison()} function and returned in the
#' \code{$distance_matrices} field.
...
#' This is generated by the \code{factorial_preprocessing()} function and
#' returned in the \code{$labels} field.
...
preText_test <- function(distance_matrices,
    ...
    compare_top_difference <- function(distance_matrices,
        ...
        rank_of_top_difference <- rank_of_top_difference /
            length(lower.tri(dist_mat))
        average_rank <- mean(rank_of_top_difference)

        return(list(ave_diff = average_rank,
                    ave_diff_SE = stats::sd(rank_of_top_difference)))
    }

    baseline <- distance_matrices[[baseline_index]]

    # make sure that we are not doing more comparisons than there are slots:
    ...
    data <- data.frame(
        Coefficient = scores[order(scores, decreasing = TRUE)],
    ...
    for (i in 1:ncol(ch)) {
        ...
        pos_sd[i] <- stats::sd(pos)
        neg_sd[i] <- stats::sd(neg)
    }

    summary_data <- data.frame(
        decision = colnames(ch),
    ...
    ret <- data.frame(
        preText_score = scores[order(scores, decreasing = TRUE)],
