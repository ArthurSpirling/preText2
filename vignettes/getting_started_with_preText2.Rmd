...
**preText2** is an updated fork of the original **preText** package, compatible
with **quanteda v4+**. It preprocesses text 64-128 different ways and assesses
how robust your findings are to preprocessing choices.
...
most to least "unusual". Higher scores mean that specification produces results
that differ more from the consensus.

The **regression coefficient plot** shows which individual preprocessing decisions
have the largest effects. Steps with coefficients far from zero are the ones
that matter most for your corpus.

Our general advice: select a preprocessing specification motivated by theory,
then for steps with significant coefficients, replicate your analysis across
combinations of those steps.
...
- UK Manifestos data now accessed via `quanteda.corpora` instead of bundled
